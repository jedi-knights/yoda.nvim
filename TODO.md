...
- [ ] **Swap `neospec` (custom Go binary test runner) → `plenary.nvim`** for the test suite. Rationale: `plenary.nvim` requires zero out-of-band install step — lazy.nvim fetches it like any other plugin dependency — whereas `neospec` always needs a separate install step before `make test` works (today that's `go install` per this repo's Makefile; neospec's own README also documents `brew install jedi-knights/tap/neospec`, but that formula is not currently published in `jedi-knights/homebrew-tap` — verified empty `Formula/` listing and no `brew search` hit as of 2026-08-18). Correcting a prior draft of this doc: "no Go toolchain requirement" was not accurate as the reason — neospec does support a non-Go install path in principle, it's just not live yet, and even if it were, it'd still be an extra manual step outside the plugin-manager loop that `plenary.nvim` doesn't require. Secondary reasons still apply: matches LazyVim / every-other-Neovim-plugin convention; simpler CI matrix. Update `Makefile`, `.github/workflows/ci.yml` (currently uses `jedi-knights/neospec@v0.1.4`), `tests/minimal_init_fast.lua`, `neospec.toml` (delete).
