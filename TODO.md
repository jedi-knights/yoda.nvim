...
- [ ] **Swap `neospec` (custom Go binary test runner) → `plenary.nvim`** for the test suite. Rationale: no Go toolchain requirement for contributors; matches LazyVim / every-other-Neovim-plugin convention; simpler CI matrix. Update `Makefile`, `.github/workflows/ci.yml` (currently uses `jedi-knights/neospec@v0.1.4`), `tests/minimal_init_fast.lua`, `neospec.toml` (delete).
