-- lua/yoda/autocmds.lua
-- Non-plugin autocommands: line-number toggling, filetype detection, git
-- refresh, session hooks, resize debouncing.
--
-- Applying is an explicit call, not a require() side effect, so a starter can
-- opt out via opts.defaults.autocmds without forking the plugin.

local M = {}

--- Register yoda's autocommands. Safe to call more than once: every augroup
--- below is created with `{ clear = true }`, so a repeat call replaces its
--- own handlers rather than stacking duplicates.
function M.apply()
