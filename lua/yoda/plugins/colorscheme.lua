...
return {
  ...
  config = function()
    ...
    -- false plugins have loaded and registered their highlight groups. Applying
    -- it synchronously here (priority 1000) would run before snacks and other
    -- plugins that add or override highlights — their ColorScheme handlers
    -- would then re-fire and overwrite ours. Scheduling moves the application
    -- to after lazy.setup() completes, ensuring a single settled highlight
    -- state.
