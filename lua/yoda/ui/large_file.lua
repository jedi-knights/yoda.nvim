-- lua/yoda/ui/large_file.lua
-- Buffer, autocmd, and notification wiring for large-file detection. Pure
-- helpers (formatting, threshold check) live in yoda.core.large_file.
...
function M.enable_large_file_mode(buf, size)
  ...
  if config.show_notification then
    ...
    local size_str = core.format_size(size)
    notify.notify(
      string.format(
    ...
    if M.is_large_file(buf) then
      ...
      notify.notify(
        string.format(
          ...
          filename,
          core.format_size(size),
          core.format_size(config.size_threshold)
        ),
        "info",
