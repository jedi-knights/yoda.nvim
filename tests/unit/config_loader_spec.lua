...
describe("config_loader", function()
  ...
  describe("load_pytest_markers()", function()
    it("extracts markers from pytest.ini", function()
      local pytest_content = [[
[pytest]
markers =
    ...
    integration: Integration tests
      ...
      assert.same({ "bdd", "unit", "integration" }, markers)
    end)

    it("handles markers on same line as 'markers ='", function()
      local pytest_content = [[
[pytest]
markers = bdd unit integration
    ...
    it("returns nil when file cannot be read", function()
      ...
      local markers = config_loader.load_pytest_markers("nonexistent.ini")
      assert.is_nil(markers)
    end)

    it("returns nil when no markers found", function()
      package.loaded["plenary.path"] = {
        new = function()
          return {
            read = function()
...
markers =
    ...
    unit: Unit tests
      ...
      assert.same({ "bdd", "unit" }, markers)
    end)

    it("stops at next section", function()
      local pytest_content = [[
[pytest]
markers =
    bdd: BDD tests
    unit: Unit tests
[other_section]
    not_a_marker: Should not be included
]]
