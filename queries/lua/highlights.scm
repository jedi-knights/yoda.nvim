; Lua highlights query for Neovim 0.12.1+
;
; Replaces the built-in runtime query which references an "operator" field
; that no longer exists in the tree-sitter-lua grammar shipped with 0.12.x.
; The operator nodes are now anonymous children, so we match them as literals.
...
; Operators — match as literal tokens (no field name)
[
  "+"
  "-"
  "*"
